    ...
    public partial class SeedDemo : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
                ...
                select 'Kaunas',@UserID,getdate(),'Lithuania'
                union
                select 'Vilnius',@UserID,getdate(),'Lithuania'
                union
                select 'Klaipėda',@UserID,getdate(),'Lithuania'
                union
                select 'Panevėžys',@UserID,getdate(),'Lithuania'
                union
                select 'Tauragė',@UserID,getdate(),'Lithuania'

                --------------------------
                --Seed Properties
                --------------------------
                IF not exists (select top 1 Address from Properties where Address='Liepų g. 7')
                insert into Properties(SellRent,PropertyTypeId,FurnishingTypeId,Price,BuiltArea,Address,
                CityId,TotalFloors,ReadyToMove,EstPossessionOn,Age,Description,PostedOn,PostedBy,LastUpdatedOn,LastUpdatedBy)
                select 
                1, --Sell Rent
                ...
                ---------------------------
                IF not exists (select top 1 Address from Properties where Address='Ateities g. 4')
                insert into Properties(SellRent,PropertyTypeId,FurnishingTypeId,Price,BuiltArea,Address,
                CityId,TotalFloors,ReadyToMove,EstPossessionOn,Age,Description,PostedOn,PostedBy,LastUpdatedOn,LastUpdatedBy)
                select 
                2, --Sell Rent
